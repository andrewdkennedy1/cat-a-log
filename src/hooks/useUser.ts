...
export function useUser() {
  ...
  const restoreGoogleToken = useCallback(async () => {
    ...
    if (storedToken && !state.user.googleToken) {
      try {
        console.log('Restoring Google token from localStorage...');
        dispatch({ type: 'SET_GOOGLE_TOKEN', payload: storedToken });
        dispatch({ type: 'SET_AUTHENTICATED', payload: true });
    ...
    } else if (storedToken && state.user.googleToken && !state.user.isAuthenticated) {
  ...
  }, [dispatch, initializeGoogleDrive, state.user.googleToken, state.user.isAuthenticated]);
